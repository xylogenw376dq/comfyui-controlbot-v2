...
MENU = [
    ...
    ("free", {"ru": "🧹 Выгрузить модели из VRAM", "en": "🧹 Free VRAM (unload models)"}),
...
STRINGS = {
    "ru": {
        ...
        "lora_video_skip": "ℹ️ LoRA не применяется к video-workflow (Wan) — генерирую без неё.",
        "lora_triggers": "\n⚠️ Триггер-слова (добавь в промпт, иначе эффекта почти нет): {words}",
        "oom_hint": "🔋 Похоже, кончилась видеопамять (OOM). Помоги: уменьши разрешение/батч или запусти /free.",
        "vseconds_status": "🎥 Видео: {seconds} с × {fps} fps = {frames} кадров → {segments} сегм. (по ≤81 кадра). Изменить: /vseconds <сек>",
        "vseconds_set": "🎥 Длительность видео: {seconds} с × {fps} fps = {frames} кадров ({segments} сегм.).",
        ...
        "lora_set": "🧩 LoRA выбрана: {name} (сила {strength}). Действует до /lora off или смены.{triggers}",
...
"lora_file_missing": "⚠️ Файл LoRA «{file}» отсутствует в models/loras/ — скачай его и попробуй снова.",
        ...
        "free_done": "🧹 Память освобождена — модели выгружены из VRAM.",
    ...
    "en": {
        ...
        "lora_video_skip": "ℹ️ LoRA is not applied to video workflows (Wan) — generating without it.",
        "lora_triggers": "\n⚠️ Trigger words (add them to the prompt or the effect is barely visible): {words}",
        "oom_hint": "🔋 Looks like the GPU ran out of memory (OOM). Try a smaller resolution/batch or run /free.",
        "vseconds_status": "🎥 Video: {seconds} s × {fps} fps = {frames} frames → {segments} segments (≤81 frames each). Change: /vseconds <sec>",
        "vseconds_set": "🎥 Video duration: {seconds} s × {fps} fps = {frames} frames ({segments} segments).",
        ...
        "lora_set": "🧩 LoRA selected: {name} (strength {strength}). Active until /lora off or a change.{triggers}",
...
"lora_file_missing": "⚠️ LoRA file «{file}» is missing from models/loras/ — download it and try again.",
        ...
        "free_done": "🧹 Memory freed — models unloaded from VRAM.",
