...
**img2video** (`/workflow video`) — из
[Comfy-Org/Wan_2.2_ComfyUI_Repackaged](https://huggingface.co/Comfy-Org/Wan_2.2_ComfyUI_Repackaged):
модель `wan2.2_ti2v_5B_fp16.safetensors` (10 ГБ) в `diffusion_models/`, энкодер
`umt5_xxl_fp8_e4m3fn_scaled.safetensors` (6.7 ГБ) в `text_encoders/`, VAE
`wan2.2_vae.safetensors` (1.4 ГБ, именно 2.2 — у неё сжатие 16×) в `vae/`.
На карте с 8 ГБ VRAM клип 1280×704, 49 кадров (~2 с) генерируется ~7 минут со стримингом весов.
